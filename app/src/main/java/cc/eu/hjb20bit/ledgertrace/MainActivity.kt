...
class MainActivity : AppCompatActivity() {
    ...
    private fun nav(): View {
        ...
        val items = listOf(
            "首页" to Page.HOME,
            "余额" to Page.BALANCES,
            "记账" to Page.TRANSACTIONS,
            "固定收支" to Page.RECURRING,
            "更多" to Page.MORE
        )
        items.forEach { (label, target) ->
            ...
            val icon = ImageView(this).apply {
                setImageResource(iconFor(label))
                scaleType = ImageView.ScaleType.CENTER_INSIDE
                layoutParams = FrameLayout.LayoutParams(dp(24), dp(24), Gravity.CENTER)
            }
            val text = tv(label, 11f, if (target == page) primary else muted, target == page).apply {
                gravity = Gravity.CENTER
                includeFontPadding = false
                layoutParams = LinearLayout.LayoutParams(-1, dp(20))
            }
            iconSlot.addView(icon)
            b.addView(iconSlot)
            b.addView(text)
            b.setOnClickListener { page = target; render() }
    ...
    private fun render() {
        ...
        navViews.forEach { (icon, label, target) -> val color = if (target == page) primary else muted; icon.setColorFilter(color); label.setTextColor(color); label.setTypeface(null, if (target == page) android.graphics.Typeface.BOLD else android.graphics.Typeface.NORMAL) }
    ...
    private fun renderHome() {
        ...
        val accountsWithSnapshots = s.accounts.filter { account ->
            account.active && s.balances.any { it.accountId == account.id }
        }
        val total = accountsWithSnapshots.sumOf { account ->
            s.balances.filter { it.accountId == account.id }.maxByOrNull { it.date }?.amountCents ?: 0
        }
        ...
        if (accountsWithSnapshots.isNotEmpty()) {
            section("账户余额") { page = Page.BALANCES; render() }
            val accountCard = card(Color.WHITE)
            accountsWithSnapshots.forEach { account ->
                val latest = s.balances.filter { it.accountId == account.id }.maxByOrNull { it.date }!!
                accountCard.addView(accountRow(account.name, latest.amountCents, latest.date))
            }
    ...
    private fun renderBalances() {
        ...
        section("区间比较", null)
        val availableDates = s.balances.map { it.date }.distinct().sorted()
        if (availableDates.isEmpty()) {
            content.addView(empty("记录余额后才能进行区间比较"))
            return
        }
        val start = snapshotDateEdit("起始日期", availableDates.first(), availableDates)
        val end = snapshotDateEdit("结束日期", availableDates.last(), availableDates)
        ...
        row.addView(end, LinearLayout.LayoutParams(0, dp(54), 1f))
        content.addView(row)
        val result = tv("选择日期后，按账户计算净变化", 13f, muted, false)
        content.addView(button("比较余额变化", -1) {
            val startDate = start.text.toString()
            val endDate = end.text.toString()
            if (startDate > endDate) toast("起始日期不能晚于结束日期") else result.text = compare(startDate, endDate)
        }, margins(0, 10, 0, 0))
        content.addView(result, margins(4, 12, 4, 0))
    }

    private fun renderTransactions() {
        val s = vm.state.value
        val formCard = card(Color.WHITE).apply { setPadding(dp(16), dp(12), dp(16), dp(14)) }
        ...
        val date = dateEdit("日期", today())
        val note = edit("备注（可选）", "")
        listOf(type, name, amount, account, date, note).forEach { formCard.addView(it) }
        formCard.addView(button("保存记录", -1) {
            val cents = MoneyFormatter.parseCents(amount.text.toString())
            if (name.text.isNullOrBlank() || cents == null) {
                toast("请填写项目名称和合法金额")
                return@button
            }
            vm.saveTransaction(
                TransactionEntity(
                    ...
                    type = if (type.selectedItemPosition == 0) "INCOME" else "EXPENSE",
        ...
        content.addView(formCard, margins(0, 14, 0, 0))
        section("历史流水", null)
        val list = card(Color.WHITE)
        s.transactions.forEach { transaction ->
            list.addView(transactionRow(transaction).apply { setOnClickListener { transactionActions(transaction) } })
        }
        if (s.transactions.isEmpty()) list.addView(empty("还没有实际收入或支出"))
    ...
    private fun transactionRow(t: TransactionEntity) = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; setPadding(dp(14), dp(11), dp(14), dp(11)); addView(tv(if (t.type == "INCOME") "入" else "出", 13f, if (t.type == "INCOME") green else red, true), LinearLayout.LayoutParams(dp(28), -2)); val info = LinearLayout(this@MainActivity).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(8), 0, 0, 0); layoutParams = LinearLayout.LayoutParams(0, -2, 1f) }; info.addView(tv(t.title, 14f, ink, true)); info.addView(tv(t.date, 11f, muted, false)); addView(info); addView(tv((if (t.type == "INCOME") "+ " else "− ") + MoneyFormatter.format(t.amountCents), 14f, if (t.type == "INCOME") green else red, true)) }
    ...
    private fun showTransactionDialog(existing: TransactionEntity?) {
        ...
        val type = Spinner(this).apply {
            adapter = ArrayAdapter(this@MainActivity, android.R.layout.simple_spinner_dropdown_item, arrayOf("收入", "支出"))
            setSelection(if (existing.type == "EXPENSE") 1 else 0)
        }
        val name = edit("项目名称", existing.title)
        val amount = edit("金额，例如 0.07", "%.2f".format(Locale.US, existing.amountCents / 100.0)).apply {
            inputType = InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_FLAG_DECIMAL
        }
        val account = edit("账户（可选）", existing.accountId?.let { id -> state.accounts.firstOrNull { it.id == id }?.name } ?: "")
        val date = dateEdit("日期", existing.date)
        val note = edit("备注（可选）", existing.note)
        listOf(type, name, amount, account, date, note).forEach { box.addView(it) }
        dialog("编辑流水", box) {
            val cents = MoneyFormatter.parseCents(amount.text.toString())
            if (name.text.isNullOrBlank() || cents == null) {
                toast("请填写项目名称和合法金额")
    ...
    private fun compare(start: String, end: String): String {
        val state = vm.state.value
        val accountsWithSnapshots = state.accounts.filter { account ->
            account.active && state.balances.any { it.accountId == account.id }
        }
        return accountsWithSnapshots.joinToString("\n") { account ->
            val begin = state.balances.firstOrNull { it.accountId == account.id && it.date == start }
            val finish = state.balances.firstOrNull { it.accountId == account.id && it.date == end }
            if (begin == null || finish == null) {
                "${account.name}：数据不足"
            } else {
                val change = finish.amountCents - begin.amountCents
                "${account.name}：${if (change >= 0) "+" else ""}${MoneyFormatter.format(change)}"
            }
        }
    }
    ...
    private fun snapshotDateEdit(hint: String, value: String, dates: List<String>) = edit(hint, value).apply {
        isFocusable = false
        setCompoundDrawablesWithIntrinsicBounds(0, 0, R.drawable.ic_calendar, 0)
        setOnClickListener {
            AlertDialog.Builder(this@MainActivity)
                .setTitle(hint)
                .setItems(dates.toTypedArray()) { _, which -> setText(dates[which]) }
                .show()
        }
    }
